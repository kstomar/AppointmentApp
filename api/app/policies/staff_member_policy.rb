class StaffMemberPolicy < ApplicationPolicy
  ...
  def update?
    (admin_or_above? && can_manage_business?) || is_self?
  end
  ...
  class Scope < Scope
    def resolve
      if user.super_admin?
        ...
        scope.joins(:business).merge(
          Business.joins(:staff_members).where(staff_members: { user_id: user.id })
        )
      else
        scope.none
      end
