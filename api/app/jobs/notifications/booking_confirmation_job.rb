module Notifications
  class BookingConfirmationJob < ApplicationJob
    ...
    def send_email_notification(booking)
      return unless user_prefers_channel?(booking.client, 'booking_confirmation', 'email')

      service = EmailService.new
      result = service.send_booking_confirmation(booking)
    ...
    def send_sms_notification(booking)
      ...
      return unless user_prefers_channel?(booking.client, 'booking_confirmation', 'sms')
    ...
    def create_in_app_notification(booking)
      Notification.create!(
        ...
        body: "Your booking for #{booking.service.name} on #{booking.start_at.strftime('%B %d at %I:%M %p')} has been confirmed.",
    ...
    def create_notification_record(booking, channel, result)
      Notification.create!(
        ...
        subject: 'Booking Confirmed',
        body: "Booking confirmation for #{booking.confirmation_code}",
