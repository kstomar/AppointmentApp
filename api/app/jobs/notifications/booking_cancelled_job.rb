module Notifications
  class BookingCancelledJob < ApplicationJob
    ...
    def send_email_notification(booking)
      return unless user_prefers_channel?(booking.client, 'booking_cancelled', 'email')

      service = EmailService.new
      result = service.send_booking_cancelled(booking)
    ...
    def send_sms_notification(booking)
      ...
      return unless user_prefers_channel?(booking.client, 'booking_cancelled', 'sms')
    ...
    def create_in_app_notification(booking)
      Notification.create!(
        ...
        body: "Your booking for #{booking.service.name} on #{booking.start_at.strftime('%B %d at %I:%M %p')} has been cancelled.",
    ...
    def notify_staff(booking)
      ...
      Notification.create!(
        ...
        status: 'sent',
        subject: 'Booking Cancelled',
        body: "Booking with #{booking.client.full_name} for #{booking.service.name} on #{booking.start_at.strftime('%B %d at %I:%M %p')} has been cancelled.",
        sent_at: Time.current
      )
    end
    ...
    def create_notification_record(booking, user, channel, result)
      Notification.create!(
        ...
        subject: 'Booking Cancelled',
        body: "Cancellation notice for booking #{booking.confirmation_code}",
